...
void stackDump(Stack_t *stk, char *file = __FILE__, size_t line = __LINE__) {
    ...
    for (size_t i = stk->size + 2; i < stk->maxDataSize; i++) {
        printf("        [%ld] = %d\n", i, stk->data[i]);
    }
...
bool stackOK(Stack_t *stk) {
    ...
    if (hash((const __uint8_t *) stk->data, stk->maxDataSize, 100) != stk->hash) {
        stackDump(stk);
        return false;
    }

    if (stk->err != 0 || stk->data[0] != 'A' || stk->data[stk->size] != 'Z') {
        stackDump(stk);
        return false;
    }
...
int stackConstruct(Stack_t *stk) {
    ...
    stk->data[0] = 'A';
    stk->data[1] = 'Z';
    stk->hash = hash((const __uint8_t *) stk->data, stk->maxDataSize, 100);
...
int stackPush(Stack_t *stk, stkElem_t value) {
    ...
    stk->data[stk->size + 2] = stk->data[stk->size + 1];
